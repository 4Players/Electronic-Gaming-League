		...
		{if $smarty.section.round.index <= $cuptree_maxrounds}
							...
							***************************************}
							...
							{if $cup->participant_type == $parttype_member && !$is_ticket }
								{if $cup_parmatches[round][$memb_cnt]->is_winner}
									<A href="{$url_file}page=cms.member.profile&member_id={$cup_parmatches[round][$memb_cnt]->participant_id}"><b> {$cup_parmatches[round][$memb_cnt]->participant_name} </b></a>
								{else}
									<A href="{$url_file}page=cms.member.profile&member_id={$cup_parmatches[round][$memb_cnt]->participant_id}"> {$cup_parmatches[round][$memb_cnt]->participant_name} </a>
								{/if}
							...
							{elseif $cup->participant_type == $parttype_team && !$is_ticket }
								...
								{if $cup_parmatches[round][$memb_cnt]->is_winner }
									{if $cup_parmatches[round][$memb_cnt]->participant_clan_id != $Smarty.const.EGL_NO_ID} <A class="href_cup" href="{$url_file}page=cms.clan.profile&clan_id={$cup_parmatches[round][$memb_cnt]->participant_clan_id}"><b> {$cup_parmatches[round][$memb_cnt]->participant_clan_tag}</b> </a> {#arrow_db_right#} {/if}
									<A class="href_cup" href="{$url_file}page=cms.team.profile&team_id={$cup_parmatches[round][$memb_cnt]->participant_id}"><b> {$cup_parmatches[round][$memb_cnt]->participant_name} </b></a>
								{else}
									...
									<A class="href_cup" href="{$url_file}page=cms.team.profile&team_id={$cup_parmatches[round][$memb_cnt]->participant_id}"> {$cup_parmatches[round][$memb_cnt]->participant_name} </a>
								{/if}
			...
			{else}
				{if $cuptree_vsmask[round][member]}
					...
					<td align='right'>  <A href='{$url_file}page=cms.match.admin&match_id={$cup_matches[round][$vs_cnt]->match_id}'><b>VS.</b></a> </td>
