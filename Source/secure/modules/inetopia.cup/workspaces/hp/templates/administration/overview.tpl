 ...
 <tr><td bgcolor="#FFFFFF">
	...
	{section name=cup loop=$cups}
	 <tr onClick="if (tableShowHide{$smarty.section.cup.index}.style.display == 'none') tableShowHide{$smarty.section.cup.index}.style.display = 'block'; else tableShowHide{$smarty.section.cup.index}.style.display = 'none';">
	 	<td colspan="2"><h3>{$cups[cup]->name}</h3> [ 	<A href="{$url_file}page={$CURRENT_MODULE_ID}:administration.cuptree&cup_id={$cups[cup]->id}"><B>Turnierbaum</b></a> | 
	 													<A href="{$url_file}page={$CURRENT_MODULE_ID}:administration.participants&cup_id={$cups[cup]->id}"><B>Teilnehmer</b></a> ]
		</td>
		 ...
		 <table width="100%"><tr>
		  ...
		  <tr><td colspan="3" align="right">	
			  {if !$cups[cup]->encounts_created} <a href="{$url_file}page={$url_page}&cup_id={$cups[cup]->id}&a=start"><img border=0 src="images/buttons/new/bt_start.gif">{/if}
		  		<a href="{$url_file}page={$CURRENT_MODULE_ID}:administration.admin&cup_id={$cups[cup]->id}"><img border=0 src="images/buttons/new/bt_admin.gif">
