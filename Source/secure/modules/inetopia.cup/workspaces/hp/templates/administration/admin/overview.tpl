			 ...
			 <tr bgcolor="{#clr_content#}">
				 <td colspan="2">
					...
					{if !$cup->encounts_created}
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="Erstellen & Starten" link="`$url_file`page=`$url_page`&cup_id=`$cup->id`&a=start"}</td>
					 {/if}
					{if !$cup->finished && $cup->encounts_created}
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="Beenden" link="`$url_file`page=`$url_page`&cup_id=`$cup->id`&a=finish"}</td>
					{/if}
					{if $cup->finished}
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="Eröffnen" link="`$url_file`page=`$url_page`&cup_id=`$cup->id`&a=open"}</td>
					{/if}
					{if $cup->encounts_created && !$cup->finished}
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="Resetten" link="javascript: if( confirm('Wollen Sie wirklich das Turnier resetten?\\nAlle Begegnungen werden dadurch gelöscht.')) document.location='`$url_file`page=`$url_page`&cup_id=`$cup->id`&a=reset';"}</td>
					{/if}
					</tr></table>
				 </td>
			 </tr>
