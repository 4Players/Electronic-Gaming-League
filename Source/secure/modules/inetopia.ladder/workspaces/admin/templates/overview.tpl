		   ...
		   {if $index < sizeof($games) }
		   		<td align="center"> 
		   			<table bgcolor="#000000" cellpadding="1" cellspacing="0"><tr><td>
		   				<A href="{$url_file}page={$CURRENT_MODULE_ID}:gameladders&game_id={$games[$index]->id}"><img onmousemove="javascript:detailwindow_showdiv('dwindow{$games[$index]->id}');" onmouseout="javascript:detailwindow_hidediv('dwindow{$games[$index]->id}');" border="0" src="{$PATH_GAMES}small/{$games[$index]->logo_small_file}" width="90" height="120"/></a>
					</td></tr></table>		
