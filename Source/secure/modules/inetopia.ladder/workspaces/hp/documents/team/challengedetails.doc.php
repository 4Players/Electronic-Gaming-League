<?php
	...
	# fetch object data
	$oChallenge	= $cLadderSystem->GetChallengeDetails( PARTTYPE_TEAM, $iChallengeId );
	$oTeam = $cTeam->GetTeam( (int)$iTeamId );
	...
	if( $_GET['a'] == "accept" )
	{
		...
		// check changes
		if( $oChallenge->react_id == $iTeamId ) // my reacttion?
			...
			$oChallenge->challenge_time != $challenge_time )
		{
			...
			PageNavigation::Location( $gl_oVars->sURLFile.'?page='.$gl_oVars->sURLRealPage.'&challenge_id='.$iChallengeId.'&team_id='.$iTeamId );
		}		
		else{
			...
			else
			{
				...
				$m_re = $cMatch->AddMatch(	array(	 	'module_id'				=> MODULEID_INETOPIA_LADDER,
														...
														'participant_type' 		=> PARTTYPE_TEAM,
				...
				$iMatchId	= $gl_oVars->cDBInterface->InsertId();	#  gets currently created sql-id
				...
				else
				{
					...
					else
					{
						...
						PageNavigation::Location( $gl_oVars->sURLFile.'?page='.$gl_oVars->sURLRealPage.'&team_id='.$iTeamId.'&challenge_id='.$iChallengeId );
					}//if
	...
	# fetch comments to that challenge
