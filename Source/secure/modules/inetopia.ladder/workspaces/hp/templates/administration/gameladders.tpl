		   ...
		   {if $index < sizeof($ladders) }
		   		<td align="center"> 
		   			<table cellpadding="1" cellspacing="4"><tr><td>
		   				<A href="{$url_file}page={$CURRENT_MODULE_ID}:administration.admin.overview&ladder_id={$ladders[$index]->id}"><img onmousemove="javascript:detailwindow_showdiv('dwindow{$ladders[$index]->id}');" onmouseout="javascript:detailwindow_hidediv('dwindow{$ladders[$index]->id}');" border="0" src="images/modules/inetopia_ladder/logo_ladder.gif" /></a>
					</td></tr></table>		
