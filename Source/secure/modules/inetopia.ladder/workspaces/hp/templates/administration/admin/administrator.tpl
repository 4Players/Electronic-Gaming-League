 	  ...
 	  <tr><td bgcolor="#FFFFFF">
		 	...
		 	<tr bgcolor="{#clr_content#}">
				<td width="200"><b>Bereits eingetragen:</b></td>
			 	...
			 	{section name=admin loop=$ladderadministrator}
			 		<A title="Mehr Informationen?" href="{$url_file}page=cms.member.central&member_id={$ladderadministrator[admin]->member_id}"><b>{$ladderadministrator[admin]->nick_name}</b></a>[ <A title="Admin entfernen" href="{$url_file}page={$url_page}&cup_id={$cup->id}&permission_id={$ladderadministrator[admin]->id}&a=delete_admin"><img border="0" src="images/admin/button_cancel_small.gif"/></a> ]
			...
			<tr bgcolor="#FFF8F2">
			 	<td colspan="2>" align="right">
			 		<table>
					<form name="fname" action="{$url_file}page={$url_page}&game_id={$game->id}&ladder_id={$ladder->id}&a=add_admin" method="POST">
			 		 <tr>
					 	<td><b>Admin</b></td>
					 	<td width="100">
					 	...
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR link="javascript:document.fname.submit()" caption="abschicken"}</td>
					 </tr>
					</form>
					<form name="fid" action="{$url_file}page={$url_page}&game_id={$game->id}&ladder_id={$ladder->id}&a=add_admin" method="POST">
					 <tr>
					 	<td><b>Admin-ID</b></td>
					 	<td><input name="admin_id" type="text" size="10" class="egl_text"/></td>
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR link="javascript:document.fid.submit()" caption="abschicken"}</td>
					 </tr>
					</form>
			 		</table>
			 	</td>
