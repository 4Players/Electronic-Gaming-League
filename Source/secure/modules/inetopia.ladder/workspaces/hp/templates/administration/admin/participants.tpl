 	  ...
 	  <tr><td bgcolor="#FFFFFF">
			...
			<tr bgcolor="{#clr_content#}"
				...
				{if $ladder->participant_type == $smarty.const.PARTTYPE_MEMBER}
					<td><a href="{$url_file}page=administration.member.central&member_id={$ladder_participants[part]->participant_id}">{$ladder_participants[part]->participant_name|strip_tags|stripslashes}</a></td>
				{elseif $ladder->participant_type == $smarty.const.PARTTYPE_TEAM}
					<td>{if $ladder_participants[part]->participant_clan_id}<a href="{$url_file}page=administration.clan.central&clan_id={$ladder_participants[part]->participant_clan_id}">{$ladder_participants[part]->participant_clan_tag}</a>&nbsp;&raquo;&nbsp;{/if}<A href="{$url_file}page=administration.team.central&team_id={$ladder_participants[part]->participant_id}">{$ladder_participants[part]->participant_name|strip_tags|stripslashes}</a></td>
				{/if}
