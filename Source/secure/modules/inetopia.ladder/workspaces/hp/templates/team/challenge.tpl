...
<!--# CHALLENGE-TYPE#-->
{if $PROCESS == 0}
	<form name="f" method="POST" action="{$url_file}page={$url_page}&ladderpart_id={$_get.ladderpart_id}&team_id={$_get.team_id}&process=1">
		...
		<td>
		 		<table cellpadding="5">
		 		 <tr>
		 		 	...
		 		 	<td><a href="{$url_file}page=team.info&team_id={$opponent->participant_id}">{$opponent->participant_name|strip_tags|stripslashes}</a></td>
		 		 </tr>		 		
		 		 <tr>
...
{elseif $PROCESS == 1}
	...
	{if $challenge_type == "randommap" }
		...
		<form name="f" method="POST" action="{$url_file}page={$url_page}&ladderpart_id={$_get.ladderpart_id}&ctype={$challenge_type}&team_id={$_get.team_id}&process=2">
		<table width="100%" cellpadding="5">
		 <tr>
		 	<td>
	...
	{elseif $challenge_type == "singlemap" }
		...
		<table width="100%" cellpadding="5">
		 ...
		 <tr>
		 	<td align="center">
		 		<table><tr>
		 		<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c1101 link="`$url_file`page=`$url_page`&ladderpart_id=`$_get.ladderpart_id`&team_id=`$_get.team_id`&process=0"}</td>
		 		<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c1100 link="javascript:document.f.submit();"}</td>
		 		</tr></table>
		 	</td>
		  </tr>
...
<!--# ACCEPT CHALLENGE #-->
{elseif $PROCESS == 2}
		 <form name="f" method="POST" action="{$url_file}page={$url_page}&ladderpart_id={$_get.ladderpart_id}&ctype={$challenge_type}&team_id={$_get.team_id}&map1={$map1}&process=3">
		<table width="100%" cellpadding="5">
		 <tr>
		 	<td>
		 		...
		 		<table cellpadding="5">
		 		 ...
		 		 <tr>
		 		 	...
		 		 	{if $challenge_type == "randommap"}<td>{$LNG_BASIC.c1200}</td>{/if}
		 ...
		 <tr>
		 	<td align="center">
		 		<table><tr>
		 		<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c1101 link="`$url_file`page=`$url_page`&ladderpart_id=`$_get.ladderpart_id`&team_id=`$_get.team_id`&process=1&ctype=`$_get.ctype`&map1=`$map1`"}</td>
		 		<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c1100 link="javascript:document.f.submit();"}</td>
		 		</tr></table>
		 	</td>
		  </tr>
