...
<form name="f" action="{$url_file}page={$url_page}&challenge_id={$challenge->id}&team_id={$team->id}&a=accept" method="POST">
 <table width="95%" align="center">
  <tr><td>
	<table cellpadding="5">
	 <tr>
	 	<td><b>{$LNG_MODULE.c9015}:</b></td><!--# LADDER #-->
	 ...
	 <tr>
	 	<td><b>{$LNG_MODULE.c9033}:</b></td><!--# MODE/MODUS #-->
	 ...
	 {else}
		 <tr>
		 	<!--# MAP-1 NAME INPUT by challenger #-->
		 	<td><b>{$LNG_MODULE.c9034}1:</b></td>
		 	{if !$CHALLENGE_LOCKED}

		 		{if $challenge->react_id == $team->id && ($team->id == $challenge->challenger_id || $challenge->challenge_type == $smarty.const.CHALLENGETYPE_SINGLE_MAP) }
		 		 	{if isset($MAPS)}
	 ...
	 {/if}
	  ...
	  <!--# MAP2  #-->
	 {if $challenge->challenge_type == $smarty.const.CHALLENGETYPE_DOUBLE_MAP}
	 <tr>
	 	<td><b>{$LNG_MODULE.c9034|strip_tags}2:</b></td> <!--# Second map input #-->
	 	{if !$CHALLENGE_LOCKED}
	 	
		 		{if $challenge->react_id == $team->id && $team->id == $challenge->opponent_id }
		 		 	{if isset($MAPS)}
	 ...
	 <tr>
 	 <!--# Challenger name <nick-name> #-->
		 {if $challenge->challenger_id == $team->id}<td><b>{$LNG_MODULE.c9017}:</b></td>{/if}
		 {if $challenge->opponent_id == $team->id}<td><b>{$LNG_MODULE.c9016}:</b></td>{/if}
			 {if $challenge->challenger_id == $team->id}<td><A href="{$url_file}page=team.info&team_id={$challenge->opponent_id}">{$challenge->opponent_name|strip_tags|stripslashes}</a></td>{/if}
		 	{if $challenge->opponent_id == $team->id}<td><A href="{$url_file}page=team.info&team_id={$challenge->challenger_id}">{$challenge->challenger_name|strip_tags|stripslashes}</a></td>{/if}
	 </tr>
	 <tr>
	 	<td><b>{$LNG_MODULE.c9036}:</b></td> <!--# Challenge date #-->
	 	<td>
	 	{if !$CHALLENGE_LOCKED}
	 		{if $challenge->react_id == $team->id}
 ...
 <td width="100" valign="top">
	...
	<table cellpadding="5">
	<!--# Accept #-->
	{if !$CHALLENGE_LOCKED} {if $challenge->react_id == $team->id} <tr><td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_MODULE.c9101 link="javascript:document.f.submit();"}</td></tr> {/if} {/if}

	<!--# Deny #-->
	{if !$CHALLENGE_LOCKED} {if $challenge->react_id == $team->id} <tr><td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_MODULE.c9102 link="`$url_file`page=`$CURRENT_MODULE_ID`:team.deny&challenge_id=`$challenge->id`&team_id=`$team->id`"}</td></tr> {/if} {/if}
	...
	<!--# jump to [challenge]overview #-->
	 <tr><td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_MODULE.c9103 link="`$url_file`page=`$CURRENT_MODULE_ID`:team.challengelist&team_id=`$team->id`"}</td></tr>
	</table>
	...
	<table border="0" width="100%" cellpadding="3" cellspacing="0">
	 <tr bgcolor="{#clr_content#}">
	 	<td align="right"> <A href='{$url_file}page={$url_page}&challenge_id={$challenge->id}&team_id={$team->id}&comment=write'> <b>{$LNG_MODULE.c9104} {#clip_start#}{$comment_count}{#clip_end#}</b> </a> </td> 
	 </tr>
	</table>
