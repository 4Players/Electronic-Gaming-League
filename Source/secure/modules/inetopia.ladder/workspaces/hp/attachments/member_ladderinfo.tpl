{if sizeof($attached_teamladders) OR sizeof($attached_1on1ladders)}
...
<table border="0" cellpadding="2" cellspacing="0" width="100%">
 {section name=ladder loop=$attached_1on1ladders}
  <tr>
	<td valign="top">
		<A class="league_entry" href="{$url_file}page=gameview.summary&game_id={$attached_1on1ladders[ladder]->game_id}">{if strlen($attached_1on1ladders[ladder]->game_token) > 0}{$attached_1on1ladders[ladder]->game_token|strip_tags|stripslashes}{else}{$attached_1on1ladders[ladder]->game_name|strip_tags|stripslashes}{/if}</a> : <A class="league_entry" href="{$url_file}page={$ATTACHED_MODULE_ID}:overview&ladder_id={$attached_1on1ladders[ladder]->id}">{$attached_1on1ladders[ladder]->name|strip_tags|stripslashes}</a>
		( {$attached_1on1ladders[ladder]->points|tointeger} {$LNG_MODULE.c1002})
		<br/>

		{if sizeof($attached_1on1ladder_matches[ladder]) > 0}
			<table width="100%" cellpadding="5" cellspacing="1">
			<tr bgcolor="{#clr_content_border#}"><td colspan="3"><b>{$LNG_MODULE.c1000}:</b></td></tr>
			{section name=match loop=$attached_1on1ladder_matches[ladder]}
			<tr bgcolor="{#clr_content#}">
				<td width="50" align="center">
				{if $attached_1on1ladder_matches[ladder][match]->status == $smarty.const.MATCH_RUNNING}
					<font color="#0000A8"><b>{$LNG_MODULE.c1013}</b></font>
				{else}
					{if $member_details->id == $attached_1on1ladder_matches[ladder][match]->winner_id}
						<font color="#00A800"><b>{$LNG_MODULE.c1010}</b></font>
					{elseif $attached_1on1ladder_matches[ladder][match]->winner_id != $smarty.const.EGL_NO_ID}
				...
				<td>
				<a href="{$url_file}page=match.info&match_id={$attached_1on1ladder_matches[ladder][match]->id}">{$attached_1on1ladder_matches[ladder][match]->challenger_name|strip_tags|stripslashes} vs.{$attached_1on1ladder_matches[ladder][match]->opponent_name|strip_tags|stripslashes}</a>
	
				<!--## SHOW GIVEN POINTS ##-->
				{if $attached_1on1ladder_matches[ladder][match]->status == $smarty.const.MATCH_CLOSED}
					{if $member_details->id == $attached_1on1ladder_matches[ladder][match]->challenger_id}
						( {marknumber number=$attached_1on1ladder_matches[ladder][match]->challenger_points color1="#00A800" color2="#A80000" color3="#A8A8A8"}	)
					{/if}
					{if $member_details->id == $attached_1on1ladder_matches[ladder][match]->opponent_id}
						( {marknumber number=$attached_1on1ladder_matches[ladder][match]->opponent_points color1="#00A800" color2="#A80000" color3="#A8A8A8"}	)
					{/if}
				{/if}
				
				</td>
				<td width="150">{date timestamp=$attached_1on1ladder_matches[ladder][match]->challenge_time}</td>
			</tr>
			{/section}
			</table>
		{else}
	...
	<td width="100" valign="bottom">
 			<table border="0" width="100%" cellpadding="5" cellspacing="0">
 			 <tr>
 			 	<td>{include file="buttons/bt_universal.tpl" caption=$LNG_MODULE.c1001 link="javascript:document.location.href='`$url_file`page=`$ATTACHED_MODULE_ID`:member.challenge&ladderpart_id=`$attached_1on1ladders[ladder]->part_id`';" color=$GLOBAL_COLOR}
 			 </tr>
 			</table>
	</td>
 	</tr>
  ...
  {section name=ladder loop=$attached_teamladders}
 	<tr>
 		<td valign="center">
 			...
 			,{$LNG_BASIC.c1010} <A href="{$url_file}page=team.info&team_id={$attached_teamladders[ladder]->team_id}">{$attached_teamladders[ladder]->team_name|strip_tags|stripslashes}</a>
