<?php
	...
	if( $_GET['a'] == 'createpoll' )
	{
		/*
		$poll_obj = array( 	'cat_id' => $iCatId,
	...
	elseif( $_GET['a'] == 'add_admin' )
	{
		...
		if( $oAdmin )
		{
			...
			$add_obj =  array(	"admin_id"		=> $iAdminId,
								"permissions" 	=> "polls",
								"module_id"		=> MODULEID_INETOPIA_POLLS,
								"data"			=> $iCatId );
			// admin exists?
	...
	$oPollCategories 	= $cMyCategory->GenerateTree( $iCatId );
	$aCategoryPolls 	= $cPolls->GetCategoryPolls( $iCatId, 100 );
	
	$aCategoryAdministrator = array();
	$aAdministrator = array();
	
	# fetch admins
	$aCategoryAdministrator = $cPolls->GetCategoryAdministrator( $iCatId );
	...
	$aPath = $cMyCategory->GetPath( $oPollCategories->oProperties->id );
	$gl_oVars->cTpl->assign( "path", $aPath );
	
	

	$gl_oVars->cTpl->assign( 'pollcategories', $oPollCategories );
	$gl_oVars->cTpl->assign( 'categorypolls', $aCategoryPolls );
