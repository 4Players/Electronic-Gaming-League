...
<form name="fpoll" action="{$url_file}page={$url_page}&poll_id={$poll->id}&cat_id={$_get.cat_id}" method="POST">
				...
				<tr bgcolor="{#clr_content#}">
					...
					<td><select style="width:300;" name="poll_cat_id" class="egl_select">
							{defun name="recursion" catroot=$categoryroot level="0"}
							    <option value="{$catroot->oProperties->id}" {if $catroot->oProperties->id == $poll->cat_id}selected{else}disabled{/if} >{section name=c loop=$level}&nbsp;&nbsp;&nbsp;{/section} {$catroot->oProperties->name}</option>
				 ...
				 <tr bgcolor="{#clr_content#}">
					<td colspan="2" align="right">
					 	...
					 	<table width="100%">
					 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="abschicken" link="javascript:document.fpoll.submit();"}</td>
				...
				<tr bgcolor="{#clr_content#}">
				  <td>
					 	...
					 	<tr bgcolor="{#clr_content#}">
					 		...
					 		<td width="1%"><A title="Löschen" href="{$url_file}page={$url_page}&poll_id={$poll->id}&answer_id={$answers[ans]->id}&a=delete_answer"><img border=0 src="images/hpadmin/button_cancel.gif"/></a></td>
					 	</tr>
					 	{/section}
				 ...
				 <tr bgcolor="{#clr_content#}">
				 	<td colspan="2" align="right">{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="abschicken" link="javascript:document.fpoll.submit();"}</td>
				 </tr>
				</table>
			 </td></tr>
			</table>
