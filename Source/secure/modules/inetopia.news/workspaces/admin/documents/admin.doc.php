<?php
	...
	if( $_GET['a'] == 'go' )
	{
		...
		if( $cNews->SetNewData( $update_obj, $iNewsId ) )
		{
			...
			$gl_oVars->cTpl->assign( 'msg_text',  	'News erfolreich ge�ndert' );
	...
	elseif ($_GET['a'] == 'delete')
	{
		...
		if( $_POST['delete_accepted'] == 'yes' )
		{
			if( $cNews->DeleteNews( $iNewsId ) )
			{
				...
				$gl_oVars->cTpl->assign( 'success',  			true );
				$gl_oVars->cTpl->assign( 'msg_type',  	'success' );
				$gl_oVars->cTpl->assign( 'msg_title',  	'News gel�scht' );
				$gl_oVars->cTpl->assign( 'msg_text',  	'Die News wurde erfolgreich gel�scht' );
