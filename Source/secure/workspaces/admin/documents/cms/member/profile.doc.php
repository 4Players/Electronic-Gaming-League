	...
	if( $oMemberData )
	{
		...
		else if( $_GET['a'] == 'change_profile' )
		{
			...
			while( list($var_name, $var_value) = each($_POST) ) 
			{
				...
				if( Isset( $aMemberData[$var_name] ) )
				{
					...
					if( $aMemberData[$var_name] != $var_value )
					{
						...
						{
							$update_obj[$var_name]=$var_value;
							if( $var_name=='password' ) $update_obj[$var_name]=md5($var_value);
							// ....
						}
					}//if
				}//if
			...
			if( !$qre )
			{
				...
				$gl_oVars->cTpl->assign( 'msg_text', 	'DB-Error:' . $gl_oVars->cDBInterface->GetLastError() );
			}
			else
			{
				...
				else
				{
					...
					$gl_oVars->cTpl->assign( 'msg_title', 	'Daten erfolgreich geändert' );
					$gl_oVars->cTpl->assign( 'msg_text', 	'Sie haben Ihre Einstellungen erfolgreich geändert!');
				}//if
