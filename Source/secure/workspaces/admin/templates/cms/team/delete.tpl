...
{if !$success}
	...
	<div>
		<table cellpadding="5">
		 <tr>
			<td><b>Bitte Team `{$team->name|strip_tags|stripslashes}` jetzt löschen</b></td>
			<td>{include file="buttons/bt_universal.tpl" caption="weiter" link="javascript:document.location.href='`$url_file`page=`$url_page`&team_id=`$team->id`&a=delete';"}</td>
		 </tr>
		</table>
	</div>
{/if}