<?php
	...
	while( list($var_name, $var_value) = each($_POST) ) 
	{
		...
		{
			$aSearchObj[$var_name] = $gl_oVars->cDBInterface->EscapeString( $var_value );
			$brun_search = true;
		}//if
	}//while	
	...
	{
		# create results
		$aResults = $gl_oVars->cDBInterface->FetchArrayObject($gl_oVars->cDBInterface->Query( $gl_oVars->cDBInterface->CreateSelectQuery( $GLOBALS['g_egltb_clan_accounts'], $aSearchObj, true, NULL, ' LIMIT 0,50', false, '$VAR="$VALUE"' ) ));
