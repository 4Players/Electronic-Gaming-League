<?php
		...
		$acc_data 	= $gl_oVars->cMember->GetClanAccount((int)$_GET['clan_id']);
		if( $acc_data ) $acc_id	 	= $acc_data->id;
		$path_logos = EGLDIR_LOGOS.'clans/';
		...
		if( $_GET['a'] == 'go' && $acc_id > 0 )
		{
			...
			if( $_POST['upload_logo_delete'] == '1' )
			{
				...
				{
					# delete file 
					if( file_exists( $path_logos . $acc_data->logo_file ) )  _unlink( $path_logos . $acc_data->logo_file  );
					
					$up_obj 			= NULL;
					$up_obj->logo_file	= 'non';
					
					$b_success 			= false;
					$b_success 			= $cClan->SetClanData($up_obj, $acc_id );
			...
			{			
				...
				# delete old data
				if( $acc_data->logo_file != 'non' && strlen($acc_data->logo_file))	
