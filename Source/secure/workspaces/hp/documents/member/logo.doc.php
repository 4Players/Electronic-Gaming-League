<?php
		...
		#$cLogo = new Logo( $gl_oVars->cDBInterface, $GLOBALS['g_egltb_logo_pool']);
		...
		#if( $_GET['type'] == 'member' )
		#{
			$acc_data 	=  $gl_oVars->oMemberData;
			$acc_id	 	= $acc_data->id;
			$path_logos = EGLDIR_LOGOS.'members/';
		#}
		/*
		if( $_GET['type'] == 'clan' )
		{
			$acc_data 	= $g_cMember->GetClanAccount((int)$_GET['clan_id']);
			if( $acc_data ) $acc_id	 	= $acc_data->id;
			$path_logos = 'files/logo_pool/clans/';
		}*/
		...
		if( $_GET['a'] == 'go' && $acc_id > 0 )
		{
			...
			if( $_POST['upload_logo_delete'] == '1' )
			{
				...
				if( $acc_data->logo_file != 'non' )
				{
					...
					$b_success 			= false;
					$b_success = $gl_oVars->cMember->SetMemberData($up_obj );
					...
					if( $b_success )
					{
						...
						$gl_oVars->cTpl->assign( 'msg_text', 	$gl_oVars->aLngBuffer['basic']['c4574'] );
					}//if update profil
				}
			...
			{			
				...
				{	
					...
					if( $b_success )
					{
						...
						$gl_oVars->cTpl->assign( 'msg_text', 	$gl_oVars->aLngBuffer['basic']['c4575'] );
