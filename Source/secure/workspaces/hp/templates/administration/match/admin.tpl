  			  	...
  			  	<td valign="top" width="50%"> 	
  			  		...
  			  		<table border="0" cellpadding="0" cellspacing="4" width="100%%">
  			  		 <tr>
  			  		   	<td valign="top" align="center" width="50%"> 
					  			...
					  			{else if $match->participant_type == $smarty.const.PARTTYPE_TEAM }
					  				{if $challenger->participant_logo_file && $challenger }
  			  		 ...
  			  		 <tr>
  			  		 	...
  			  		 	<b>
						{if $match->participant_type == $smarty.const.PARTTYPE_MEMBER }
	 						<A class="href_orange" href="{$url_file}page=administration.member.central&member_id={$challenger->participant_id}">	{$challenger->participant_name} </a>
	 					{else if $match->participant_type == $smarty.const.PARTTYPE_TEAM }
	 						...
	 						<A class="href_orange" href="{$url_file}page=administration.team.central&clan_id={$challenger->participant_clan_id}&team_id={$challenger->participant_id}">{$challenger->participant_name} </a>
					 	{/if}
  			  		 	</b>
  			  		 		 </td>
  			  		 </tr>
  			  	...
  			  	<td align="center" valign="top">
  			  		...
  			  		<table border="0" cellpadding="0" cellspacing="2" width="100%">
  			  		 ...
  			  		 <tr>
  			  		 	...
  			  		 	<b>
	 					{if $match->participant_type == $smarty.const.PARTTYPE_MEMBER }
	 						<A class="href_orange" href="{$url_file}page=administration.member.central&member_id={$opponent->participant_id}">	{$opponent->participant_name} </a>
	 					{else if $match->participant_type == $smarty.const.PARTTYPE_TEAM }
	 						...
	 						<A class="href_orange" href="{$url_file}page=administration.team.central&clan_id={$opponent->participant_clan_id}&team_id={$opponent->participant_id}">{$opponent->participant_name} </a>
					 	{/if}
  			  		 		</b>
  			  		 		</td>
  			  		 </tr>
			  	...
			  	<form name="fparams" action="{$url_file}page={$url_page}&match_id={$match->id}&a=update_parameters" method="POST">
		 			...
		 			<table border="0" cellpadding="5" cellspacing="1" width="100%">
		  			 ...
		  			 <tr>
		  			 	...
		  			 	<td colspan="2>">{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="abschicken" link="javascript:document.fparams.submit();"}</td>
		  			 </tr>
		  			</table>  
			  	</form>
			  	</td></tr>
			  ...
			  <tr>
				...
				{section name=map_res loop=$match->num_maps}
					...
					{if $display_detailed_rounds }
						<tr><td colspan="4">
							<table border="0" cellpadding="0" cellspacing="8" width="100%" style="background-repeat:no-repeat;" background="images/match_admin_map_bg.gif" bgcolor="#F5F3EF">
							 <tr>
				...
				{/section}
				<tr ><td colspan="4">
					<table border="0" cellpadding="0" cellspacing="10" width="100%" style="background-repeat:no-repeat;" background="images/match_admin_map_bg.gif" bgcolor="#F5F3EF">
					 <tr>
  			    ...
  			    <tr>
  			    	...
  			    	<td colspan="4" align="left"> {include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="abschicken" link="javascript:document.fresults.submit();"}</td>
				</tr>
				</form>
	  			...
	  			<table border="0" cellpadding="2" cellspacing="0" width="100%">
	  			 {section name=media loop=$media_files}
	  			 	<tr>
	  			 		...
	  			 		<td> 
	  			 			<A target="_blank" href="files/media_pool/{$media_files[media]->file_name}"> <b>{$media_files[media]->name|strip_tags|stripslashes}</b> </a> <br/> 
	  			 			<table cellpadding="0" cellspacing="0" width="100%">
	  			 				<tr><td style="font-size:11px"> {filesize file="`$path_media`/`$media_files[media]->file_name`"} Kb, {date timestamp=$media_files[media]->created}, by <A href="{$url_file}page=administration.member.central&member_id={$media_files[media]->member_id}">{$media_files[media]->member_nick}</a>  </td></tr>
 ...
 <tr>
  <td colspan="2">
	  ...
	  <div align="left">
	  	<table>
	  	 <tr>
	  	 	{if $match->evaluated} <td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="zurücksetzen" link="javascript:document.location='`$url_file`&page=`$url_page`&match_id=`$match->id`&a=restore_evaluation';"}</td>{/if}
	  	 	{if !$match->evaluated} <td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption="auswerten" link="javascript:document.location='`$url_file`&page=`$url_page`&match_id=`$match->id`&a=evaluate_match';"}</td>{/if}
	  	 </tr>
	  	</table>
	  </div> 
	  	...
	  	<table border="0" cellpadding="5" cellspacing="0" width="100%" bgcolor="{#clr_content_border#}">
	  	 <tr>
	  	 	...
	  	 	{if $reports[rep]->participant_id == $challenger->participant_id }
		  	 	{if $match->participant_type == $smarty.const.PARTTYPE_TEAM }
		  	 		{$challenger->participant_clan_tag}:<i>{$challenger->participant_name|strip_tags|stripslashes}</i>
		 		{else if $match->participant_type == $smarty.const.PARTTYPE_TEAM }
		 			{$challenger->participant_name}
		 		{/if}
	  	 	{else if $reports[rep]->participant_id == $opponent->participant_id }
		  	 	{if $match->participant_type == $smarty.const.PARTTYPE_TEAM }
		  	 		{$opponent->participant_clan_tag}:<i>{$opponent->participant_name|strip_tags|stripslashes}</i>
		 		{else if $match->participant_type == $smarty.const.PARTTYPE_MEMBER }
		 			{$opponent->participant_name}
		 		{/if}
	  	 	{/if}
			</b>	 		
		 	by <A href="{$url_file}page=administration.member.central&member_id={$reports[rep]->member_id}">{$reports[rep]->member_nick|strip_tags|stripslashes}</a>
