...
<h2>{$LNG_BASIC.c8267}</h2>
{include file="devs/hr_black.tpl" type="1" width="100%"}

<table cellpadding="5">
 <tr>
 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c8251  	link="javascript:document.location.href='`$url_file`page=administration.protests.overview';"}</Td>
 	<td>{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c8266 	link="javascript:document.location.href='`$url_file`page=administration.protests.delete&protest_id=`$protest->id`';"}</Td>
 </tr>
</table>
	...
	<table border="0" width="100%">
		<tr>
			...
			{if $protest->match_id != $smarty.const.EGL_NO_ID } 
				<A href="{$url_file}page=administration.match.admin&match_id={$protest->match_id}" ><img border=0 src="images/protests_match_enabled.gif"/></a> 
				<br/>{$LNG_BASIC.c1029}: {$protest->match_id}
			{else}
				<img border=0 src="images/protests_match_disabled.gif"/>
				<br/><i>{$LNG_BASIC.c8258}G</i>
			{/if}

			</td>
			<td valign="top">
				<br/><h2>{$LNG_BASIC.c8201}: {if strlen($protest->subject)>0}{$protest->subject|strip_tags|stripslashes}{else}<i>KEIN BETREFF</i>{/if}</h2><br/>
				...
				<table border="0" width="100%" cellpadding="5" cellspacing="1" bgcolor="{#clr_content_border#}">
				 ...
				 <tr bgcolor="{#clr_content#}" align="center">
				 	<td><textarea name="admin_text" class="egl_text" style="width:100%;" rows="10">{$protest->admin_text}</textarea>
				 </tr>
				 <tr bgcolor="{#clr_content#}" align="right">

				 {if $protest->administrated}
		 			<td align="center"><input type="checkbox" class="egl_checkbox" value="yes" name="administrated" checked /><b>{$LNG_BASIC.c8268}</b></td>
				 {else}
		 			<td align="center"><input type="checkbox" class="egl_checkbox" value="yes" name="administrated" /><b>{$LNG_BASIC.c8268}</b></td>
				 {/if}
				 </tr>
				 <tr bgcolor="{#clr_content#}" align="right">
		 			<td align="center">{include file="buttons/bt_universal.tpl" color=$GLOBAL_COLOR caption=$LNG_BASIC.c1018 link="javascript:document.f.submit();"}</td>
				 </tr>
				 </table>
				 </form>
