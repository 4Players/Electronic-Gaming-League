...
function callback( $oVars, $params )
{
	...
	if( $params['current_permission']->module_id == EGL_NO_ID )
	{
		return 1; // ja access
	}
	# 2. Fall: module_id, kein data(module_entity_id)
	else if( $params['current_permission']->module_id != EGL_NO_ID &&  (int)$params['current_permission']-data == 0 )
	{
		...
		if( $iMatchId )
		{
			...
			if( $params['current_permission']->module_id == $oMatch->module_id )
			{
				return 1;
			}//if
		}//if
	...
	else if( $params['current_permission']->module_id != EGL_NO_ID &&  (int)$params['current_permission']->data > 0 )
	{
		...
		if( $iMatchId )
		{
			...
			# correct match ?
			if( $oMatch->module_entry_id == $params['current_permission']->data )
			{
				return 1;
			}
